...
| `SUPABASE_KEY` | **anon public** API 키 (Settings → API) | ✅ |

`NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`만 넣어 둔 경우에도 서버에서 동일하게 읽습니다.

> ⚠️ 위 값들은 프로젝트 루트의 **`.env.local` 파일**에 넣습니다(폴더 아님). `.gitignore`로 커밋되지 않습니다. `npm run dev`를 **다시 시작**해야 새 변수가 반영됩니다.
