...
function buildEmailHtml(ideas: ServiceIdea[], comment: string): string {
...
<body style="margin:0;padding:0;background:#F8F9FA;">
  <div style="max-width:600px;margin:0 auto;padding:32px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">

    <!-- 헤더 -->
    <div style="text-align:center;padding:40px 0 32px;">
      <h1 style="font-size:24px;color:#111827;margin:0 0 8px;font-weight:800;">
        맞춤 AI 서비스 아이디어 진단 결과
      </h1>
      <p style="color:#6B7280;font-size:15px;margin:0;line-height:1.5;">
        바이브 코딩 툴로 2주 안에 만들 수 있는 서비스예요
      </p>
    </div>
    ...
    <!-- CTA 섹션 -->
    <div style="text-align:center;margin-top:36px;padding:32px 24px;background:#FFFFFF;border-radius:16px;border:1px solid #E5E7EB;">
      <p style="margin:0 0 4px;font-size:14px;color:#9CA3AF;">
        <s>정가 299,000원</s>
      </p>
      <p style="margin:0 0 8px;font-size:26px;font-weight:800;color:#00C471;">
        얼리버드 99,000원
      </p>
    ...
    <div style="text-align:center;margin-top:32px;padding-top:20px;border-top:1px solid #E5E7EB;">
      <p style="margin:0;font-size:12px;color:#9CA3AF;">
        이 이메일은 AI 서비스 아이디어 진단 결과를 안내하기 위해 발송되었습니다.
      </p>
    </div>
...
export async function POST(req: Request) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: email,
      subject: "[나도코딩] AI 서비스 아이디어 진단 결과가 도착했어요",
      html: buildEmailHtml(ideas, comment),
    });
